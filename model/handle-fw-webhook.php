    ...
    try {
      ...
      if ($existingTx && mysqli_num_rows($existingTx) > 0) {
        $updateTxSql = "UPDATE transactions
                        SET user_id = $user_id, amount = $total_amount, charge = $charge, profit = $profit, refund = $refund_applied, status = 'successful', medium = 'FLUTTERWAVE'
                        WHERE ref_id = '$tx_ref'";
        if (!mysqli_query($conn, $updateTxSql)) {
          throw new Exception('Failed to repair transaction: ' . mysqli_error($conn));
        }
      } else {
        $insertTxSql = "INSERT INTO transactions (ref_id, user_id, amount, charge, profit, refund, status, medium) VALUES ('$tx_ref', $user_id, $total_amount, $charge, $profit, $refund_applied, 'successful', 'FLUTTERWAVE')";
      ...
      sendMail('FLW Webhook: Transaction Error', $e->getMessage() . ' Ref: ' . $tx_ref, 'webhook@nivasity.com');
      continue;
    }
